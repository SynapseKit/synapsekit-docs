...
- **9 new vector stores** (#888) — async-first adapters for **Turbopuffer**, **Azure AI Search**, **Vertex AI Vector Search**, **SingleStore**, **TiDB Vector**, **Couchbase**, **SurrealDB**, **Deep Lake**, and **MyScale**, each behind its own extra and lazily imported. All implement the full [`VectorStore`](/docs/rag/vector-stores) contract (`add`/`search`/`search_mmr`/`save`/`load`) via a shared mixin (document normalization, metadata round-trip, MMR over provider candidates, portable JSON snapshots, and sync-client offloading through `asyncio.to_thread`). SQL-backed adapters parameterize queries, validate identifiers, and cast `top_k`; Vertex persists a local datapoint→document map since its response returns IDs only. Brings the total to **32 vector-store backends**. Contributed by [@DhruvGarg111](https://github.com/DhruvGarg111).
- **[Guardrails & policy middleware](/docs/guardrails)** (#892) — a unified, auditable I/O governance layer that runs pre-call (input) and post-call (output) checks on any LLM with four enforcement modes: `block`, `redact`, `flag`, and `require_human`. Ships input guards (`PromptInjectionGuard`, `JailbreakGuard`, `TopicGuard`, `MaxCostGuard`) and output guards (`PIIRedactionGuard`, `ToxicityGuard`, `CitationRequiredGuard`, `SchemaGuard`). A `GuardrailPolicy` composes guards over both stages, and `GuardedLLM` wraps any `BaseLLM` so the policy applies transparently on `generate`/`stream`. Three ready-made compliance rulepacks (HIPAA/GDPR/PCI-DSS). Every decision emits a signed, replayable audit record and a `guardrail.*` SynapseKit Live event. Pure-Python, zero new dependencies.
- **First-class embeddings & reranker provider layer** (#886) — a uniform async `BaseEmbeddings` contract (`embed`/`embed_one`/`embed_batch`, L2-normalized `(N, D)` float32) across **9 providers**: OpenAI, Cohere, Mistral, Voyage, Jina, Nomic, mixedbread, Hugging Face, and Gemini, plus a `Reranker` contract with three new hosted rerankers (Jina, Voyage, mixedbread) alongside the existing Cohere/cross-encoder. Wired into SynapseKit Live via `embeddings.embed` / `rerank` events. Contributed by [@DhruvGarg111](https://github.com/DhruvGarg111).
- **Multi-agent orchestration eval (loops, context loss, mis-routing)** (#983) — goal-level, orchestration-aware evaluation over a framework-agnostic `RunGraph` abstraction detecting three failure modes: infinite handoff loops (`LoopDetector`), per-transfer context loss (`ContextLossDetector`, plus an opt-in async `LLMContextLossJudge`), and non-deterministic mis-routing (`MisroutingDetector`). `OrchestrationEvaluator` aggregates findings into a JSON/Markdown report, wired into SynapseKit Live via `orchestration.eval` events. Contributed by [@Premvkmishra](https://github.com/Premvkmishra).
- **[Dream Mode](/docs/dream)** (#749) — a local-first, explicit opt-in runner that, while the machine is idle and plugged in, replays the day's signed audit traces, distills durable lessons, proposes (never auto-applies) `LivingMemory` patches, refreshes `KnowledgeMesh` state, and emits a signed audit bundle plus a morning briefing. Every stage is gated: constructing `DreamMode` starts no background task, patches stay pending for human review, and the scheduler fails closed when power state is unknown. A per-install Ed25519 signing key makes audit bundles attestable. New `synapsekit dream run|status` CLI. Contributed by [@DhruvGarg111](https://github.com/DhruvGarg111).
- **[Ambient daemon (MVP)](/docs/ambient)** (#743) — an opt-in background daemon that watches a git repo for dirty state and the terminal for risky commands (destructive deletes, `git reset --hard`, force-push/clean), firing a notification when a risky command lands against uncommitted work. Rule-based, no ML. Command text is redacted before it enters the audit log, with a per-source privacy opt-out and an explicit shell-history disclosure notice at startup. New `synapsekit ambient start|stop|status|log` CLI. Contributed by [@Abhay-Mmmm](https://github.com/Abhay-Mmmm).
- **[Hive Mode: privacy-first pooled memory](/docs/hive)** (#748) — an opt-in way for a team, or a self-hosted community, to learn shared conventions without sharing raw memory. A local `HiveClient` mines only safe, vocabulary-level pattern keys, never filenames, paths, or excerpts, applies differential-privacy noise, and Ed25519-signs each contribution. A self-hostable `HiveAggregator` enforces a minimum-cohort suppression threshold and returns aggregate suggestions. Full transparency and withdrawal, a per-day privacy budget, and a `synapsekit hive` CLI. Contributed by [@DhruvGarg111](https://github.com/DhruvGarg111).
- **[Agent OS Shell](/docs/shell)** (#750) — a local-first hybrid shell that mixes ordinary shell syntax with quoted natural-language requests. Commands run argv-only (never `shell=True`), a fail-closed safety gate blocks destructive commands unless signed, and every run writes a signed, verifiable audit receipt. Ships an LLM planner, a rule planner, and a `synapsekit shell` / `synshell` entry point. Contributed by [@DhruvGarg111](https://github.com/DhruvGarg111).
- **[Code Archaeology Agent](/docs/agents/code-archaeology)** (#744) — reconstructs why code exists in its current form by fusing git history, local markdown notes, and opt-in Slack/email archives into a cited markdown narrative via `ArchaeologyAgent.explain(query)`. Reuses the `timetravel`, `loaders`, and `symbolic` modules with zero new hard dependencies. Contributed by [@DhruvGarg111](https://github.com/DhruvGarg111).
