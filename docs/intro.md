...
description: "SynapseKit is the async-first Python LLM framework for RAG pipelines, AI agents, and graph workflows. 2 dependencies, 46 providers, production-ready."
...
OpenAI, Anthropic, Ollama, Cohere, Mistral, Gemini, AWS Bedrock, Azure OpenAI, Vertex AI, Groq, DeepSeek, OpenRouter, Together, Fireworks, Perplexity, Cerebras, Moonshot, Zhipu, Cloudflare, AI21 Labs, Databricks, Baidu ERNIE, llama.cpp, LM Studio, Minimax, Aleph Alpha, Hugging Face, SambaNova, xAI (Grok), NovitaAI, Writer, vLLM, GPT4All, MLX, NVIDIA NIM, watsonx.ai, Snowflake Cortex, DeepInfra, Nebius, Baseten, Upstage, Reka, Hyperbolic, FriendliAI, Clarifai, Replicate: all behind `BaseLLM`. Auto-detected from the model name.

→ [LLM Provider docs](/docs/llms/overview)

### 32 vector store backends

InMemoryVectorStore (built-in, `.npz` persistence), ChromaDB, FAISS, Qdrant, Pinecone, Weaviate, PGVector, Milvus, LanceDB, SQLiteVec, MongoDBAtlas, Redis, Supabase, Cassandra, ClickHouse, DuckDB, Elasticsearch, OpenSearch, Typesense, Vespa, Zilliz, Marqo, Turbopuffer, Azure AI Search, Vertex AI Vector Search, SingleStore, TiDB Vector, Couchbase, SurrealDB, Deep Lake, MyScale, and a graph-fused `GraphVectorStore`: 32 in total, all behind `VectorStore`.

→ [Vector store docs](/docs/rag/vector-stores)

### 83 document loaders

`TextLoader`, `StringLoader`, `PDFLoader`, `HTMLLoader`, `CSVLoader`, `JSONLoader`, `YAMLLoader`, `XMLLoader`, `DiscordLoader`, `SlackLoader`, `NotionLoader`, `GoogleDriveLoader`, `GoogleSheetsLoader`, `GoogleCalendarLoader`, `DirectoryLoader`, `WebLoader`, `ExcelLoader`, `PowerPointLoader`, `DocxLoader`, `MarkdownLoader`, `AudioLoader`, `VideoLoader`, `WikipediaLoader`, `ArXivLoader`, `EmailLoader`, `ImageLoader`, `ConfluenceLoader`, `RSSLoader`, `GCSLoader`, `SQLLoader`, `GitHubLoader`, `GitLoader`, `GitRepoLoader`, `JiraLoader`, `SupabaseLoader`, `TeamsLoader`, `S3Loader`, `AzureBlobLoader`, `MongoDBLoader`, `DropboxLoader`, `LaTeXLoader`, `TSVLoader`, `RTFLoader`, `EPUBLoader`, `ConfigLoader`, `OneDriveLoader`, `ParquetLoader`, `RedisLoader`, `ElasticsearchLoader`, `DynamoDBLoader`, `YouTubeLoader`, `ObsidianLoader`, `AirtableLoader`, `SitemapLoader`, `HubSpotLoader`, `SalesforceLoader`, `BigQueryLoader`, `FirestoreLoader`, `SnowflakeLoader`, `TwitterLoader`, `RedditLoader`, `TrelloLoader`, `ZendeskLoader`, `FreshdeskLoader`, `IntercomLoader`, `HackerNewsLoader`, `PubMedLoader`, `AsanaLoader`, `BoxLoader`, `ClickUpLoader`, `DatabricksLoader`, `DeltaLakeLoader`, `FigmaLoader`, `GmailLoader`, `GraphQLLoader`, `IcebergLoader`, `KafkaLoader`, `LinearLoader`, `LocalMdLoader`, `MondayLoader`, `OpenKnowledgeFormatLoader`, `ShopifyLoader`, `ZoomLoader`.
